...
Build the actual standalone artifact with `npm run build:firebase`. With Java 21 installed, run:

```sh
npx --yes firebase-tools@15.30.0 emulators:exec --project demo-offhire --only firestore,auth "npm run test:firebase"
```

This runs 20 Firestore transaction tests, 21 source/auth integration checks, and 10 HTTP checks against the standalone production server. Test identities and the Firebase project are fictional. No test sends a phone call. Emulator success does not establish deployed IAM, real Google popup authentication, Firebase Hosting routing or a live CALL-E conversation.
